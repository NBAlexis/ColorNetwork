...
BYTE* CFile::ReadAllBytes(const TCHAR* sFilename, UINT& size)
{
    FILE* fp = NULL;
...
UBOOL CFile::WriteAllBytes(const TCHAR* sFilename, BYTE* data, UINT uiSize)
{
    FILE* fp = NULL;
...
CCString CFile::ReadAllText(const TCHAR* sFilename)
{
    FILE* fp = NULL;
    fopen_s(&fp, sFilename, _T("rb"));
    if (NULL == fp)
...
UBOOL CFile::WriteAllText(const TCHAR* sFilename, const CCString& data)
{
    return WriteAllBytes(sFilename, (BYTE*)data.c_str(), data.GetLength() * sizeof(TCHAR));
}

UBOOL CFile::AppendAllText(const TCHAR* sFilename, const CCString& data)
{
    std::ofstream file;
...
UBOOL CFile::IsFileExist(const CCString& sFileName)
{
...
OFSTREAM* CFile::OpenFileToLoad(const CCString& sFilename, UBOOL bBinary)
{
    OFSTREAM* file = new OFSTREAM(sFilename, std::ios::in | (bBinary ? std::ios::binary : 0));
    return file;
}

OFSTREAM* CFile::OpenFileToSave(const CCString& sFilename, UBOOL bBinary, UBOOL bAppend)
{
    OFSTREAM* file = new OFSTREAM(sFilename, 
        std::ios::out | (bBinary ? std::ios::binary : 0) | (bAppend ? std::ios::app : 0));
    return file;
}
