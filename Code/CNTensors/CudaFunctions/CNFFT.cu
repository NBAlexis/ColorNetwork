...
_kernelScale(CNComplex* res, Real fScale)
{
    intokernal;
...
__global__ void _CLG_LAUNCH_BOUND
_kernelCopyElementSU3ToC(deviceSU3* su3, CNComplex* res, BYTE idx)
{
    intokernal;
...
__global__ void _CLG_LAUNCH_BOUND
_kernelCopyElementCToSU3(deviceSU3* su3, CNComplex* res, BYTE idx)
{
    intokernal;
...
UBOOL CCLGFFTHelper::FFT3DWithXYZ(CNComplex* copied, TArray<INT> dims, UBOOL bForward)
{
    cufftHandle plan3d;
...
*/
UBOOL CCLGFFTHelper::FFT3DWithXYZW(CNComplex* copied, TArray<INT> dims, UBOOL bForward)
{
    cufftHandle plan4d1;
    INT n[3] = { dims[0], dims[1], dims[2] };
...
UBOOL CCLGFFTHelper::FFT4DWithXYZW(CNComplex* copied, TArray<INT> dims, UBOOL bForward)
{
    cufftHandle plan4d1;
...
UBOOL CCLGFFTHelper::FFT3D(CNComplex* copied, UBOOL bForward, EFFT_Scale eScale)
{
    TArray<INT> dims;
...
UBOOL CCLGFFTHelper::FFT4D(CNComplex* copied, UBOOL bForward, EFFT_Scale eScale)
{
    TArray<INT> dims;
    dims.AddItem(static_cast<INT>(_HC_Lx));
    dims.AddItem(static_cast<INT>(_HC_Ly));
...
void CCLGFFTHelper::CheckBuffer()
{
    ...
    {
        checkCudaErrors(cudaMalloc((void**)& m_pDeviceBuffer, _HC_Volume * sizeof(CNComplex)));
    }
}
...
void CCLGFFTHelper::GenerateTestArray(CNComplex * hostArray, INT iSize)
{
...
void CCLGFFTHelper::PrintTestArray4D(CNComplex* hostArray)
{
    appGeneral(_T("{\n"));
...
void CCLGFFTHelper::PrintTestArray3D(CNComplex* hostArray)
{
    appGeneral(_T("{\n"));
    for (INT i = 0; i < _tfftMX; ++i)
    {
...
void CCLGFFTHelper::TestFFT()
{
    ...
    CNComplex* dD3Res;
    CNComplex* hD3Res = (CNComplex*)malloc(_tfftMX * _tfftMY * _tfftMZ * sizeof(CNComplex));
    CNComplex* hD3Source = (CNComplex*)malloc(_tfftMX * _tfftMY * _tfftMZ * sizeof(CNComplex));
    checkCudaErrors(cudaMalloc((void**)& dD3Res, _tfftMX * _tfftMY * _tfftMZ * sizeof(CNComplex)));
    ...
    checkCudaErrors(cudaMemcpy(dD3Res, hD3Source, _tfftMX * _tfftMY * _tfftMZ * sizeof(CNComplex), cudaMemcpyHostToDevice));
    TArray<INT> dims3d;
    dims3d.AddItem(_tfftMX);
    dims3d.AddItem(_tfftMY);
    dims3d.AddItem(_tfftMZ);
    CCLGFFTHelper::FFT3DWithXYZ(dD3Res, dims3d, TRUE);
    checkCudaErrors(cudaMemcpy(hD3Res, dD3Res, _tfftMX * _tfftMY * _tfftMZ * sizeof(CNComplex), cudaMemcpyDeviceToHost));
    appGeneral(_T("res3d="));
    PrintTestArray3D(hD3Res);


    CNComplex* dD4Res;
    CNComplex* hD4Res = (CNComplex*)malloc(_tfftMX * _tfftMY * _tfftMZ * _tfftMT * sizeof(CNComplex));
    CNComplex* hD4Source = (CNComplex*)malloc(_tfftMX * _tfftMY * _tfftMZ * _tfftMT * sizeof(CNComplex));

    checkCudaErrors(cudaMalloc((void**)& dD4Res, _tfftMX * _tfftMY * _tfftMZ * _tfftMT * sizeof(CNComplex)));

    GenerateTestArray(hD4Source, _tfftMX * _tfftMY * _tfftMZ * _tfftMT);
    appGeneral(_T("sour="));
    PrintTestArray4D(hD4Source);
    checkCudaErrors(cudaMemcpy(dD4Res, hD4Source, _tfftMX * _tfftMY * _tfftMZ * _tfftMT * sizeof(CNComplex), cudaMemcpyHostToDevice));
    TArray<INT> dims;
    ...
    checkCudaErrors(cudaMemcpy(hD4Res, dD4Res, _tfftMX * _tfftMY * _tfftMZ * _tfftMT * sizeof(CNComplex), cudaMemcpyDeviceToHost));
    appGeneral(_T("res1="));
